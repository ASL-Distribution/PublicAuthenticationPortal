...
using System.Data;
using PublicAuthenticationPortal.Models;
    ...
    public partial class CRUD : System.Web.UI.Page
    {
        ...
        private void AddSearchControls()
        {
            ...
            foreach (var searchField in searchFields)
            {
                ...
                label.Text = searchField;
                label.ID = searchTbl.ID + "|" + searchField + "|L";

                tableCell.Controls.Add(label);

                Literal literal = new Literal();
                literal.Text = "<br />";
                literal.ID = searchTbl.ID + "|" + searchField + "|Li";

                tableCell.Controls.Add(literal);

                TextBox textBox = new TextBox();
                textBox.ID = searchTbl.ID + "|" + searchField + "|TB";

                tableCell.Controls.Add(textBox);

                tableCell.CssClass = "SearchCell";
        ...
        private void SearchButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < searchTbl.Rows[0].Cells.Count; i++)
                {
                    var cell = searchTbl.Rows[0].Cells[i];

                    var textBox = (TextBox)cell.Controls[2];
                    var searchField = searchFields[i];
                    var value = textBox.Text.Replace("'", "''");

                    searchClauses += searchField + " like '%" + value + "%', ";
                }

                searchClauses = searchClauses.Substring(0, searchClauses.Length - 2);

                Session.Add("ASL_ENTERPRISE_TABLE_SEARCH_CLAUSE", searchClauses);
        ...
        private void GetSearchFields()
        {
            ...
            {
                var searchFieldsS = Request.QueryString["search"];
                searchFields.AddRange(searchFieldsS.Split(','));
            }
        }
        ...
        {
            Session.Remove("ASL_ENTERPRISE_EDITING_NOTIFICATION_ENTITY_ID");
            Session.Remove("ASL_ENTERPRISE_TABLE_SEARCH_CLAUSE");
        }
        ...
        private void LoadData()
        {
            ...
            if (Session["ASL_ENTERPRISE_TABLE_SEARCH_CLAUSE"] != null)
            {
                searchClauses = (string)Session["ASL_ENTERPRISE_TABLE_SEARCH_CLAUSE"];
            }

            var ds = SqlDB.GetData("select '', * from [" + tableName + "]" + searchClauses);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                || table.Columns[columnIndex].DataType == typeof(decimal))
                                {
                                    ...
                                    textBox.Text = row.Cells[columnIndex].Text == "&nbsp;" ? "" : row.Cells[columnIndex].Text.Replace("&amp;", "&");
